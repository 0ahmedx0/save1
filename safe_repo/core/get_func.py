...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            if await ask_for_split(sender, file, duration):
                return  # إذا تم تقسيم الفيديو، لا حاجة لرفع الفيديو الأصلي

            if msg.media == MessageMediaType.VIDEO and msg.video.mime_type in ["video/mp4", "video/x-matroska"]:
                try:
                    # إتمام رفع الفيديو بعد تقسيمه (إذا تم تقسيمه)
                    if not await ask_for_split(sender, file, duration):  # إذا لم يتم تقسيم الفيديو
...
async def split_video(file, num_parts, duration, chat_id):
    ...
    """
    try:
        # إذا كانت المدة أقل من الحد الأدنى للأجزاء (مثلاً 60 ثانية)، لا يتم التقسيم
        if duration < 60 or num_parts < 1:
            await app.send_message(chat_id, "❌ الفيديو قصير جدًا أو عدد الأجزاء غير صالح.")
            return
